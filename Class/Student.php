...
<?php
    ...
    class Student
    {
        ...
        public function student_show_courses_to_complete($student_id)
        {
            $result;
            ...
            $size = sizeof($array1);
            $course_complete = $this->student_course_completed($student_id);
            $array_send = array();
        
            for($i = 0; $i < $size; $i++)
            {
                $bool = false;
                foreach($course_complete as $row)
                {
                   
                    if($array1[$i] == $row['COURSE_CODE'] || $array2[$i] == $row['COURSE_CODE']  )
                    {
                        $bool = true;
                    }
                } 
                if(!$bool)
                {
                    if(!empty($array1[$i]))
                    {
                        if(!empty($array2[$i]))
                        {
                           $array_val = $array1[$i] . " OR " . $array2[$i];
                           $array_send[] = $array_val;     
                        }
                        else
                        {
                            $array_send[] = $array1[$i];
